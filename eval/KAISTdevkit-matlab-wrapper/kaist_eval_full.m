    ...
    for s=1:length(setIds)
        for v=1:length(vidIds{s})
            for i=skip-1:skip:99999
                ...
                num = num + 1;
                [~, x1, y1, w, h, score] = textread(detName, '%s %f %f %f %f %f');
                for j = 1:length(score)
                    fprintf(fidA, '%d,%.4f,%.4f,%.4f,%.4f,%.8f\n', num, x1(j)+1, y1(j)+1, w(j), h(j), score(j));
                end
            end
        end
    end
...
#dt = load('/home/anushl9o5/Desktop/VAL/Pedestrian Detection Papers/MSDS-RCNN(11%)/MSDS-RCNN/Results/dt-MSDS_RCNN_sanitized.mat').dt;
#dt = load('/home/anushl9o5/Desktop/Mat/seg-rpn_eval.mat').dt;
