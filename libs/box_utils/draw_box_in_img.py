...
def train_draw_boxes_with_label_and_scores(img_array, boxes, labels, scores, rgb = True, in_graph=True):
    ...
    if in_graph:
        if cfgs.NET_NAME in ['resnet101_v1d', 'resnet50_v1d']:
            ...
            if rgb == True:
                img_array = img_array + np.array(cfgs.RGB_PIXEL_MEAN)
            else:
                img_array = img_array + np.array(cfgs.IR_PIXEL_MEAN)
...
def draw_boxes_with_label_and_scores(img_array, boxes, labels, scores, txt_file, img_name, in_graph=True):
    ...
    num_of_objs = 0
    if len(scores) == 0:
        txt_file.write("%s %f %f %f %f %f \n"%(img_name.split('_')[2], 0, 0, 0, 0, 0))
    for box, a_label, a_score in zip(boxes, labels, scores):

        if a_label != NOT_DRAW_BOXES:
            num_of_objs += 1
            txt_file.write("%s %f %f %f %f %f \n"%(img_name.split('_')[2], (box[0]), (box[1]), (box[2]-box[0]), (box[3]-box[1]), a_score))
            draw_a_rectangel_in_img(draw_obj, box, color=STANDARD_COLORS[a_label], width=3)
            if a_label == ONLY_DRAW_BOXES:  # -1
                continue
            elif a_label == ONLY_DRAW_BOXES_WITH_SCORES:  # -2
