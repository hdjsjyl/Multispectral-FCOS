...
from libs.detection_oprations.fcos_target import get_fcos_target_batch, get_seg_target_batch 
...
class DetectionNetwork(object):
    ...
    def rpn_cls_ctn_net(self, inputs, scope_list, reuse_flag, level):
        ...
        seg_mask = slim.conv2d(rpn_conv2d_3x3,
                               num_outputs=2,
                               kernel_size=[1, 1],
                               stride=1,
                               weights_initializer=cfgs.SUBNETS_WEIGHTS_INITIALIZER,
        ...
        tf.summary.image('seg_{}'.format(level), tf.expand_dims(tf.nn.softmax(tf.expand_dims(seg_mask[0, :, :, 1], axis=0)), axis = -1))

        rpn_ctn_scores = tf.reshape(rpn_ctn_scores, [self.batch_size, -1],
                                    name='rpn_{}_centerness_reshape'.format(level))
        return rpn_box_scores, rpn_box_probs, rpn_ctn_scores, seg_mask
    ...
    def seg_reg_net(self, inputs, scope_list, reuse_flag):
        seg_mask = inputs
        for i in range(4):
            seg_mask = slim.conv2d(inputs=seg_mask,
                                   num_outputs=256,
                                   kernel_size=[3, 3],
                                   weights_initializer=cfgs.SUBNETS_WEIGHTS_INITIALIZER,
                                   biases_initializer=cfgs.SUBNETS_BIAS_INITIALIZER,
    ...
    def rpn_net(self, feature_pyramid, suffix):
        ...
        with tf.variable_scope('rpn_net_'+suffix):
                ...
                for level, stride in zip(cfgs.LEVLES, cfgs.ANCHOR_STRIDE_LIST):
                    ...
                    if cfgs.SHARE_HEADS:
                        ...
                        scope_list = ['conv2d_3x3_cls', 'conv2d_3x3_reg', 'rpn_classification',
                                      'rpn_centerness', 'rpn_regression', 'seg_prediction']
                    else:
                        reuse_flag = None
                        scope_list = ['conv2d_3x3_cls_' + level, 'conv2d_3x3_reg_' + level,
                                      'rpn_classification_' + level, 'rpn_centerness' + level,
                                      'rpn_regression_' + level, 'seg_prediction' + level]

                    rpn_box_scores, rpn_box_probs, rpn_ctn_scores, seg_pred = self.rpn_cls_ctn_net(feature_pyramid[level],
            ...
            return all_rpn_box_scores_list, all_rpn_box_probs_list, all_rpn_cnt_scores_list, all_rpn_box_list, seg_list


    def seg_net(self, feature_pyramid, suffix):

        seg_list = []
        with tf.variable_scope('seg_net_'+suffix):
            with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY)):
                for level, stride in zip(cfgs.LEVLES, cfgs.ANCHOR_STRIDE_LIST):

                    if cfgs.SHARE_HEADS:
                        reuse_flag = None if level == 'P3' else True
                        scope_list = ['conv2d_3x3_cls', 'conv2d_3x3_reg', 'seg_prediction']
                    else:
                        reuse_flag = None
                        scope_list = ['conv2d_3x3_cls_' + level, 'conv2d_3x3_reg_' + level,
                                      'seg_prediction_' + level]

                    seg_pred = self.seg_reg_net(feature_pyramid[level], scope_list, reuse_flag)

                    tf.summary.image('{}_seg_{}'.format(suffix, level), tf.expand_dims(tf.nn.softmax(tf.expand_dims(seg_pred[0, :, :, 1], axis=0)), axis = -1))

                    seg_list.append(seg_pred)

                #all_seg_list = tf.concat(seg_list, axis=1)
    ...
    def _seg_target(self, feature_pyramid, seg_batch, level):
        with tf.variable_scope('seg_target_'+str(level)):
            featuremap_height, featuremap_width = tf.shape(feature_pyramid[level])[1], tf.shape(feature_pyramid[level])[2]
            featuremap_height = tf.cast(featuremap_height, tf.int32)
            featuremap_width = tf.cast(featuremap_width, tf.int32)
            seg_target_batch = tf.py_func(get_seg_target_batch,
                                           inp=[seg_batch, [featuremap_height, featuremap_width]],
                                           Tout=[tf.int32])
            seg_target_batch = tf.reshape(seg_target_batch, [self.batch_size, featuremap_height, featuremap_width])
            return seg_target_batch

    def build_whole_detection_network(self, rgb_input_img_batch, ir_input_img_batch, seg_mask_batch, gtboxes_batch):
        ...
        multi_cls_score, multi_cls_prob, multi_cnt_scores, multi_box, multi_seg = self.rpn_net(feature_pyramid_multi, 'multi')
        ...
        rgb_cls_score, rgb_cls_prob, rgb_cnt_scores, rgb_box, rgb_seg = self.rpn_net(feature_pyramid_rgb, 'rgb')
        ...
        ir_cls_score, ir_cls_prob, ir_cnt_scores, ir_box, ir_seg = self.rpn_net(feature_pyramid_ir, 'ir')
        ...
        if not self.is_training:
            ...
            with tf.variable_scope('build_loss'):
                ...
                for i, levels in enumerate(cfgs.LEVLES):
                    seg_target_batch = self._seg_target(feature_pyramid_multi, seg_mask_batch, levels)
                    
                    multi_seg_loss.append(tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits = multi_seg[i], labels = seg_target_batch)))
                    rgb_seg_loss.append(tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits = rgb_seg[i], labels = seg_target_batch)))
                    ir_seg_loss.append(tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits = ir_seg[i], labels = seg_target_batch)))
                ...
                loss_dict = {
                    ...
                    'multi_seg_loss': sum(multi_seg_loss)/len(multi_seg_loss),
    ...
    def get_restorer(self):
        ...
        if checkpoint_path != None:
            ...
            for var in model_variables:
                ...
                elif var.name.startswith('RGB/resnet_v1_50/RGB/'+self.base_network_name):  # +'/block4'
                    var_name_in_ckpt = name_in_ckpt_fastrcnn_head(var)
                    rgb_nameInCkpt_Var_dict[var_name_in_ckpt] = var

                elif var.name.startswith('IR/resnet_v1_50/IR/'+self.base_network_name):  # +'/block4'
                    var_name_in_ckpt = name_in_ckpt_fastrcnn_head(var)
    ...
    def get_restorer_test(self):
        checkpoint_path = '/home/adhitya/Anush-KAIST/FCOS_Tensorflow/output/trained_weights/FCOS_Res50_20190428/coco_280000model.ckpt'
