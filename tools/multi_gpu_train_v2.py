...
from data.io.read_tfrecord_v3 import next_batch
...
def train():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        with tf.name_scope('get_batch'):
            img_name_batch, rgb_img_batch, gtboxes_and_label_batch, num_objects_batch, img_h_batch, img_w_batch, ir_img_batch, seg_mask_batch= \
        ...
        for i in range(cfgs.NUM_GPU):
            ...
            rgb_img = rgb_img_batch[start:end, :, :, :]
            ir_img = ir_img_batch[start:end, :, :, :]
            seg_mask = seg_mask_batch[start:end, :, :, :]
            if cfgs.NET_NAME in ['resnet101_v1d', 'resnet50_v1d']:
                rgb_img = rgb_img / tf.constant([cfgs.PIXEL_STD])
            ...
            inputs_list.append([rgb_img, gtboxes_and_label, num_objects, ir_img, seg_mask, img_h, img_w])
        ...
        total_loss_dict = {
            ...
            'multi_seg_loss': tf.constant(0., tf.float32),
            'rgb_seg_loss': tf.constant(0., tf.float32),
            'ir_seg_loss': tf.constant(0., tf.float32),
                    ...
                    with tf.name_scope('tower_%d' % i):
                        with slim.arg_scope(
                            ...
                            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                                ...
                                rgb_img = inputs_list[i][0]
                                ir_img = inputs_list[i][3]
                                seg_mask = inputs_list[i][4]
                                img_shape = inputs_list[i][-2:]
                                h_crop = tf.reduce_max(img_shape[0])
                                w_crop = tf.reduce_max(img_shape[1])
                                rgb_img = tf.image.crop_to_bounding_box(image=rgb_img,
                                                                    offset_height=0,
                                                                    offset_width=0,
                                                                    target_height=tf.cast(h_crop, tf.int32),
                                                                    target_width=tf.cast(w_crop, tf.int32))
                                ir_img = tf.image.crop_to_bounding_box(image=ir_img,
                                                                    offset_height=0,
                                                                    offset_width=0,
                                                                    target_height=tf.cast(h_crop, tf.int32),
                                                                    target_width=tf.cast(w_crop, tf.int32))

                                outputs = faster_rcnn.build_whole_detection_network(rgb_input_img_batch=rgb_img,
                                                                                    ir_input_img_batch=ir_img,
                                                                                    seg_mask_batch=seg_mask,
                                                                                    gtboxes_batch=gtboxes_and_label)
                                rgb_gtboxes_in_img = show_box_in_tensor.draw_boxes_with_categories(img_batch=rgb_img[0, :, :, :],
                                                                                               boxes=gtboxes_and_label[0, :, :-1],
                                                                                               labels=gtboxes_and_label[0, :, -1],
                                                                                               rgb = True)
                                rgb_gtboxes_in_img = tf.expand_dims(rgb_gtboxes_in_img, 0)
                                ir_gtboxes_in_img = show_box_in_tensor.draw_boxes_with_categories(img_batch=ir_img[0, :, :, :],
                                                                                               boxes=gtboxes_and_label[0, :, :-1],
                                                                                               labels=gtboxes_and_label[0, :, -1],
                                                                                               rgb = False)
                                ir_gtboxes_in_img = tf.expand_dims(ir_gtboxes_in_img, 0)
                                tf.summary.image('Compare/rgb_gtboxes_gpu:%d' % i, rgb_gtboxes_in_img)
                                tf.summary.image('Compare/ir_gtboxes_gpu:%d' % i, ir_gtboxes_in_img)
                                tf.summary.image('Compare/seg_mask_gpu:%d' % i, tf.expand_dims(seg_mask[0], axis = 0))
                                if cfgs.ADD_BOX_IN_TENSORBOARD:
                                    detections_rgb_img = show_box_in_tensor.draw_boxes_with_categories_and_scores(
                                        img_batch=rgb_img[0, :, :, :],
                                        boxes=outputs[0],
                                        scores=outputs[1],
                                        labels=outputs[2])
                                    detections_ir_img = show_box_in_tensor.draw_boxes_with_categories_and_scores(
                                        img_batch=ir_img[0, :, :, :],
                                        boxes=outputs[0],
                                        scores=outputs[1],
                                        labels=outputs[2],
                                        rgb = False)
                                    detections_rgb_img = tf.expand_dims(detections_rgb_img, 0)
                                    detections_ir_img = tf.expand_dims(detections_ir_img, 0)
                                    tf.summary.image('Compare/rgb_final_detection_gpu:%d' % i, detections_rgb_img)
                                    tf.summary.image('Compare/ir_final_detection_gpu:%d' % i, detections_ir_img)
        ...
        vis_restorer, lwir_restorer, restore_ckpt = faster_rcnn.get_restorer()
        ...
        with tf.Session(config=tfconfig) as sess:
            ...
            if not vis_restorer is None:
                vis_restorer.restore(sess, restore_ckpt)
                print('RGB restore model')

            if not lwir_restorer is None:
                lwir_restorer.restore(sess, restore_ckpt)
                print('IR restore model')
            '''
            if not multi_restorer is None:
                multi_restorer.restore(sess, restore_ckpt)
                print('MULTI restore model')
            '''
            #exit() #for intermediate testing ONLY
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Specify the GPU (default:0)')
