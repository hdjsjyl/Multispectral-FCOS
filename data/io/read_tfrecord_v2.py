...
def read_single_example_and_decode(filename_queue):
    ...
    return img_name, rgb_img, ir_img, gtboxes_and_label, num_objects


def read_and_prepocess_single_img(filename_queue, shortside_len, is_training):

    img_name, rgb_img, ir_img, gtboxes_and_label, num_objects = read_single_example_and_decode(filename_queue)
    ...
    if is_training:
        ...
        ir_img, _, _, _ = image_preprocess.short_side_resize(img_tensor=ir_img, gtboxes_and_label=gtboxes_and_label,
                                                                    target_shortside_len=shortside_len,
                                                                    length_limitation=cfgs.IMG_MAX_LENGTH)
        rgb_img, ir_img, gtboxes_and_label = image_preprocess.multi_random_flip_left_right(rgb_img_tensor=rgb_img, ir_img_tensor=ir_img, gtboxes_and_label = gtboxes_and_label)
    ...
    return img_name, rgb_img, ir_img, gtboxes_and_label, num_objects, img_h, img_w
...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    img_name, rgb_img, ir_img, gtboxes_and_label, num_obs, img_h, img_w = read_and_prepocess_single_img(filename_queue, shortside_len,
                                                                              is_training=is_training)
    img_name_batch, rgb_img_batch, ir_img_batch, gtboxes_and_label_batch, num_obs_batch, img_h_batch, img_w_batch = \
        tf.train.batch(
                       [img_name, rgb_img, ir_img, gtboxes_and_label, num_obs, img_h, img_w],
                       batch_size=batch_size,
                       capacity=16,
                       num_threads=16,
                       dynamic_pad=True)
    return img_name_batch, rgb_img_batch, gtboxes_and_label_batch, num_obs_batch, img_h_batch, img_w_batch, ir_img_batch
