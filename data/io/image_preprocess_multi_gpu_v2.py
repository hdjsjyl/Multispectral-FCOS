...
def multi_flip_left_to_right(rgb_img_tensor, ir_img_tensor, seg_mask_tensor, gtboxes_and_label):

    h, w = tf.shape(ir_img_tensor)[0], tf.shape(rgb_img_tensor)[1]

    rgb_img_tensor = tf.image.flip_left_right(rgb_img_tensor)
    ir_img_tensor = tf.image.flip_left_right(ir_img_tensor)
    seg_mask_tensor = tf.image.flip_left_right(seg_mask_tensor)

    xmin, ymin, xmax, ymax, label = tf.unstack(gtboxes_and_label, axis=1)
    new_xmax = w - xmin
    new_xmin = w - xmax

    return rgb_img_tensor, ir_img_tensor, seg_mask_tensor, tf.transpose(tf.stack([new_xmin, ymin, new_xmax, ymax, label], axis=0))


def multi_random_flip_left_right(rgb_img_tensor, ir_img_tensor, seg_mask_tensor, gtboxes_and_label):
    rgb_img_tensor, ir_img_tensor, seg_mask_tensor, gtboxes_and_label= tf.cond(tf.less(tf.random_uniform(shape=[], minval=0, maxval=1), 0.5),
                                            lambda: multi_flip_left_to_right(rgb_img_tensor, ir_img_tensor, seg_mask_tensor, gtboxes_and_label),
                                            lambda: (rgb_img_tensor, ir_img_tensor, seg_mask_tensor, gtboxes_and_label))

    return rgb_img_tensor, ir_img_tensor, seg_mask_tensor, gtboxes_and_label
